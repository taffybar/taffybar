module System.Taffybar.AppearanceHyprlandSpec (spec) where
...
spec = aroundAll withIntegrationEnv $ do
  it "renders a bar under Hyprland (Wayland compositor)" $ \env -> do
    goldenFile <- makeAbsolute "test/data/appearance-hyprland-bar.png"
...
renderBarScreenshot Env { envTmpDir = tmp } = do
  ...
  withProcessTerm pc $ \p -> do
    mEc <- timeout 90_000_000 (waitExitCode p)
    case mEc of
      Nothing -> do
        stopProcess p
