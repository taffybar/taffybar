...
eventLoop dn ready dispatch = logAround DEBUG "eventLoop" $ withRunInIO $ \runInIO -> do
  let mask = propertyChangeMask .|. substructureNotifyMask .|. structureNotifyMask
  withSafeX11Thread dn $ \ctx -> logAround DEBUG "eventLoop_inner" $ do
    let x11 = flip runSafeX11Sync ctx
    x11 $ do
      root <- safeRootWindow
      mkSafe2 selectInput root mask
      mkSafe1 sync False
    runInIO ready
...
interruptibleNextEvent dn = makeInterruptible . safeNextEvent
  where
    ...
    nextEventUnblocker = logAround DEBUG "nextEventUnblocker" $ do
      withSafeX11Thread dn $ \ctx -> do
        root <- runSafeX11Sync safeRootWindow ctx
        runSafeX11Sync (safeSendClientMessageEvent 0 0 root Nothing) ctx
        runSafeX11Sync (mkSafe1 sync False) ctx -- very important
